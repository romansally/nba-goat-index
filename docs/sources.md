...
| 2026-07-11 | Win Shares: corrected `player_name` "Nikola Jokic" → "Nikola Jokić" (11 rows; informational column only — the merge key is `player_id`, WS values unchanged) | T4 profiling name-consistency check | Found by the `pipeline/clean.py` cross-check against `players.csv` (nba_api static names, which carry the diacritic). Logged as finding 1 in `docs/profiling.md`; the strict-equality check now fails loudly on any future drift. |
